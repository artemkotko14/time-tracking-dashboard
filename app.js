...
function updateData(timeframe) {
    data.forEach(activity => {
        const category = document.querySelector(`.${activity.title.toLowerCase()
        .replace(" ", "")}`); //  Removes the first space in the string.
       if (category) {
            ...
            category.querySelector(".time").textContent = `${activity.timeframes[timeframe].current}hrs`;
            category.querySelector(".previousTime").textContent = `Last ${frequence} - ${activity.timeframes[timeframe].previous}hrs`;
        }
    });
}
